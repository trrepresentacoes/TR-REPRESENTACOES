...
function App(){
 ...
 {tab==='pending'&&profile.role==='admin'&&<section className="panel">
   <div className="team-head"><div><h2>🟡 Solicitações de vendedores</h2><p>Cadastros aguardando aprovação.</p></div></div>
   {pendingSignups.length===0
    ? <div className="empty">Nenhum cadastro aguardando aprovação.</div>
    : <div className="team-list">{pendingSignups.map(r=><div className="team-row" key={r.id}>
       <div className="team-avatar">{String(r.name||'?').slice(0,1).toUpperCase()}</div>
       <div className="team-info"><strong>{r.name}</strong><small>{r.email}{r.phone?` · ${r.phone}`:''}</small>{r.whatsapp_number&&<small>WhatsApp: {r.whatsapp_number}</small>}</div>
       <button onClick={()=>reviewSignup(r.id,'approved')}>✅ Aprovar</button>
       <button className="danger secondary" onClick={()=>reviewSignup(r.id,'rejected')}>❌ Recusar</button>
      </div>)}</div>}
  </section>}
