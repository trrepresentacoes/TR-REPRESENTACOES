...
function App(){
 ...
 {['dashboard','clients','ranking','report','new'].map(x=><button key={x} className={tab===x?'active':''} onClick={()=>setTab(x)}>{x==='dashboard'?'📊 Dashboard':x==='clients'?'👥 Clientes':x==='ranking'?'🏆 Ranking':x==='report'?'📅 Relatório diário':'➕ Novo cliente'}</button>)}
 ...
 <main><header><div><h1>{tab==='dashboard'?'Dashboard':tab==='clients'?'Clientes':tab==='ranking'?'Ranking':tab==='report'?'Relatório diário':'Novo cliente'}</h1><span>Atualização em tempo real</span></div></header>
